...
Zep의 node/edge 목록 API는 UUID cursor 기반 페이지네이션을 사용하며,
이 모듈은 자동 페이지 넘김 로직(단일 페이지 재시도 포함)을 캡슐화하여
호출자에게 전체 목록을 투명하게 반환한다.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """단일 페이지 요청. 실패 시 지수 백오프 재시도. 네트워크/IO 계열 일시적 오류만 재시도."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """그래프 노드를 페이지네이션으로 조회. 최대 max_items 개(기본 2000) 반환. 각 페이지 요청에 재시도 포함."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """그래프 모든 엣지를 페이지네이션으로 조회. 전체 목록 반환. 각 페이지 요청에 재시도 포함."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
